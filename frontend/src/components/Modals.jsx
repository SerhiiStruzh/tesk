...
export const ShareModal = ({ isOpen, onClose, item, onSave }) => {
  const [emailInput, setEmailInput] = useState('');
  // Локальний стан списку користувачів (копія з item.sharedWith)
  const [usersList, setUsersList] = useState([]);

  // При відкритті модалки завантажуємо поточні права
  useEffect(() => {
    if (isOpen && item) {
      setUsersList(item.sharedWith || []);
      setEmailInput('');
    }
  }, [isOpen, item]);
  ...
  const handleAddUser = () => {
    ...
    // Додаємо нового юзера з дефолтною роллю Viewer
